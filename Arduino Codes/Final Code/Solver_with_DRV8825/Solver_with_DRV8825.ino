/*creator: swapnil*/
/*implements the Layer-Wise method to solve a 3×3×3 Rubik’s Cube*/
/*we used this code with DRV8825 stepper motor drivers*/  

#define Enable_pin_W 2
#define Enable_pin_B 5
#define Enable_pin_R 8
#define Enable_pin_G 11
#define Enable_pin_Y 14
#define Enable_pin_O 17
 
/*DIR. pins to set the direction of rotation*/
#define DIR_PIN_W 3
#define DIR_PIN_B 6
#define DIR_PIN_R 9
#define DIR_PIN_G 12
...
/*STEP pins to set the speed of the stepper motors.*/ 
#define STEP_PIN_W 4
#define STEP_PIN_B 7 
#define STEP_PIN_R 10
#define STEP_PIN_G 13
#define STEP_PIN_Y 16
#define STEP_PIN_O 19

void setup()
{
  pinMode(Enable_pin_W , OUTPUT);
  pinMode(Enable_pin_B , OUTPUT);
  pinMode(Enable_pin_R , OUTPUT);
  pinMode(Enable_pin_G , OUTPUT);
  pinMode(Enable_pin_Y , OUTPUT);
  pinMode(Enable_pin_O , OUTPUT);
  ...
  pinMode(STEP_PIN_O , OUTPUT);
  
/*DRV8825 Motor Drivers are Active Low*/
  digitalWrite(Enable_pin_W , LOW);
  digitalWrite(Enable_pin_B , LOW);
  digitalWrite(Enable_pin_R , LOW);
  digitalWrite(Enable_pin_G , LOW);
  digitalWrite(Enable_pin_Y , LOW);
  digitalWrite(Enable_pin_O , LOW);
  
}
/* 200 steps equal one revolution. 180 degree will be 100 and 90 degree turn would be 50 steps.*/

void movement_W(int steps){
  int interval =2000;/*changing this interval alters the speed of the respective stepper motor*/
  for (int i=0; i<steps; i++){
    digitalWrite(STEP_PIN_W, HIGH);
    digitalWrite(STEP_PIN_W, LOW);
    delayMicroseconds(interval);
  }
}
void movement_B(int steps){
  int interval =2000;
  for (int i=0; i<steps; i++){
...
void movement_G(int steps){
  int interval =2000;
  for (int i=0; i<steps; i++){
    digitalWrite(STEP_PIN_G, HIGH);
    digitalWrite(STEP_PIN_G, LOW);
    delayMicroseconds(interval);
  }
}
void movement_Y(int steps){
  int interval =2000;
  for (int i=0; i<steps; i++){
...
void movement_O(int steps){
  int interval =2000;
  for (int i=0; i<steps; i++){
    digitalWrite(STEP_PIN_O, HIGH);
    digitalWrite(STEP_PIN_O, LOW);
    delayMicroseconds(interval);
  }
}
...
/*enter the individual faces of the cube manually/serial communication in the order proided in the image*/
/*Generally, strings are terminated with a null character (ASCII code 0). 
 This allows functions (like Serial.print()) to tell where the end of a string is. Otherwise, they would continue reading subsequent bytes of memory that aren’t actually part of the string.
...
/*function swaps two elements of an array*/
...
/*rotation of steppers of each face and corresponding color swaps to be performed.
  This ensures that the entire cube is solved only using the initial state of the faces. 
  No further input is required in between*/
void rotate_clock(char choice)
{
  if (choice=='w')
  {
   digitalWrite(DIR_PIN_W, HIGH); movement_W(50);delay(400);
  ...
  {
    digitalWrite(DIR_PIN_R, HIGH); movement_R(50);delay(400); 
    swap(red[0],red[4]);
    swap(red[7],red[5]);
    swap(red[1],red[3]);
    swap(red[0],red[6]);
  ...
  {
    digitalWrite(DIR_PIN_Y, HIGH); movement_Y(50);delay(400);
  ...
  {
    digitalWrite(DIR_PIN_O, HIGH); movement_O(50);delay(400); 
  ...
  {
   digitalWrite(DIR_PIN_G, HIGH); movement_G(50);delay(400);
  ...
  {
    digitalWrite(DIR_PIN_B, HIGH); movement_B(50);delay(400); 
    swap(blue[1],blue[7]);
    swap(blue[2],blue[6]);
    swap(blue[5],blue[3]);
    swap(blue[2],blue[0]);
...
{
    rotate_clock(a);rotate_clock(a);rotate_clock(a);/* If the situation requires that the side must be turned in anti-clockwise direction, the code will simply give the command to rotate that side in clockwise direction 3 times.*/
...
void solve_white_cross()/*making the white cross. The very first and most important step*/
{
{
...
void yellow_cross_algorithm()/*solving the yellow cross*/
{
        rotate_clock('r');
        rotate_clock('y');
        rotate_clock('g');
...
void yellow_edges_colour_arrangement()
{
  ...
  void loop() {
    /*run these steps in the order provided below and your cube would be solved in 35-40 90 degree rotations*/
  ...
  yellow_edges_colour_arrangement();
    ...
    /*If you give a wrong input, there would be infinite steps. No exception handling is used in the code. For avoiding wrong inputs, 
   use image processing so that a you can simply show all the sides of the cube to a colour sensor and it will detect the colour of each piece and then autmatically substitute the 9 elements of the six colour arrays.*/
